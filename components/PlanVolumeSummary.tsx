...
// Mirrors --error / --success / --warning from app/globals.css.
...
export default function PlanVolumeSummary({
  ...
  return (
    <div className="card" style={{ marginBottom: 24 }}>
      ...
      <p style={{ color: "var(--text-secondary)", fontSize: 12, marginBottom: 12 }}>{description}</p>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {rows.map(({ muscle, sets, target, status }) => (
          <div key={muscle} style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 13 }}>
            ...
            <div style={{ flex: 1, height: 8, background: "var(--bg-inset)", borderRadius: 4, position: "relative", overflow: "hidden" }}>
              <div
                style={{
                  height: "100%",
            ...
            <span style={{ width: 36, textAlign: "right", color: "var(--text-primary)" }}>{sets}</span>
            <span style={{ width: 70, color: "var(--text-muted)", fontSize: 11 }}>
