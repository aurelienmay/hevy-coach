...
- Choose a coherent, standard split shape for the number of available days rather than freely fragmenting a body region across multiple sessions — e.g. Full Body for 2-3 days/week, Upper/Lower (repeating as needed, such as Upper/Lower/Upper for 3 days) for 3-4 days/week, or a Push/Pull/Legs-style split for 5+ days/week. Do not split "upper body" work thinly across two separate days (e.g. Back+Biceps on one day, Chest+Shoulders+Triceps on another) when a single combined upper session would fit the time budget — that fragments frequency and volume without benefit.
- Keep exercise and working-set counts roughly even across all training days that week (e.g. not a 3-exercise day next to a 5-6 exercise day) unless a genuine constraint — a real recovery need, a stated client priority, or a thin exercise pool for that day's muscles — forces the imbalance, and if so say so explicitly in that day's rationale.`;
